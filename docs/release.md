...
Windows 可双击 `scripts/build-fpk.cmd`，或执行 `./scripts/build-fpk.ps1`，完成同等的前端构建、交叉编译、FPK 打包和产物校验；该入口仍需要 Git Bash 或 WSL 提供 POSIX `bash`。输出固定为 `dist/fpk`；如目录不可写，需先处理占用或目录权限。
